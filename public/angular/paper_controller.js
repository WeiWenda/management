 ...
 */
 //管理专利列表
 cjsApp.controller("adminPaper",['$scope','$http','$filter','pageData','getItemService',
    function($scope,$http,$filter,pageData,getItemService){
        $scope.formData = {};
        //获取管理员列表信息
        initPagination($scope,$http);
        //删除用户
        initDelOption($scope,$http,'您确认要删除选中的学术论文吗？');
        // 修改用户
        initDirections($scope,$http);
        initPersons($scope,$http);
        initProjects($scope,$http);
        initCCFLevels($scope,$http);
        $('#addNewPaper').modal({dimmer:0,closeViaDimmer: 0, width: 800});
        $('#addNewPaper').modal('toggle');
        $('#publish_time').datepicker().
            on('changeDate.datepicker.amui', function(event) {
              $scope.formData["publish_time"] = event.date;
            $scope.$apply();
            });
        ...
        $('#addNewPaper').on('open.modal.amui', function (event) {
            console.log("弹出框");
