 ...
 */
 //管理专利列表
 cjsApp.controller("adminProject",['$scope','$http','$filter','pageData','getItemService',
    function($scope,$http,$filter,pageData,getItemService){
        $scope.formData = {};
        //获取管理员列表信息
        initPagination($scope,$http);
        //删除用户
        initDelOption($scope,$http,'您确认要删除选中的科研项目吗？');
        // 修改用户
        initProjectLevels($scope,$http);
        initPersons($scope,$http);
        $('#addNewProject').modal({dimmer:0,closeViaDimmer: 0, width: 800});
        $('#addNewProject').modal('toggle');
        $('#start_time').datepicker().
            on('changeDate.datepicker.amui', function(event) {
              $scope.formData["start_time"] = event.date;
            $scope.$apply();
            });
        $('#end_time').datepicker().
            on('changeDate.datepicker.amui', function(event) {
              $scope.formData["end_time"] =event.date;
               $scope.$apply();
            });
        ...
        $('#addNewProject').on('open.modal.amui', function (event) {
            console.log("弹出框");
