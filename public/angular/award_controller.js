 ...
 */
 //管理专利列表
 cjsApp.controller("adminAward",['$scope','$http','$filter','pageData','getItemService',
    function($scope,$http,$filter,pageData,getItemService){
        $scope.formData = {};
        //获取管理员列表信息
        initPagination($scope,$http);
        //删除用户
        initDelOption($scope,$http,'您确认要删除选中的科研获奖吗？');
        // 修改用户
        initAwardLevels($scope,$http);
        initPersons($scope,$http);
        $('#addNewAward').modal({dimmer:0,closeViaDimmer: 0, width: 800});
        $('#addNewAward').modal('toggle');
        $('#time').datepicker().
            on('changeDate.datepicker.amui', function(event) {
              $scope.formData["time"] = event.date;
            $scope.$apply();
            });
        ...
        $('#addNewAward').on('open.modal.amui', function (event) {
            console.log("弹出框");
