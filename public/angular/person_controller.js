 ...
 */
 //管理专利列表
 cjsApp.controller("adminPerson",['$scope','$http','pageData','getItemService',
    function($scope,$http,pageData,getItemService){
        $scope.formData = {};
        //获取管理员列表信息
        initPagination($scope,$http);
        //删除用户
        initDelOption($scope,$http,'您确认要删除选中的人员吗？');
        $('#addNewPerson').modal({dimmer:0,closeViaDimmer: 0, width: 800});
        $('#addNewPerson').modal('toggle');
        $('#addNewPerson').on('open.modal.amui', function (event) {
            console.log("弹出框");
